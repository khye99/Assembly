...
void testByteToAscii() {
  ...
  Serial.println("Test Case: Only errors will print:");
  for (byte i=0; i<10; i++) {
    if(byteToAscii(i) != i+48) {
      Serial.print("byteToAscii has error on: ");
      Serial.println(i);
    }
//    else {
//      Serial.println(i);
...
void testAddInt8ToInt() {
  ...
  int8_t int8Values[]  = {0, 1, -1,  1, -1,  -1,  127, -128, -127, -127, 100, -100, -4, 0};
  int    int16Values[] = {0, 1,  1, -1, -1, 256, -127,  128,  128, -127, 300, 1000, -6, -100};
  int    results[]     = {0, 2,  0,  0, -2, 255,    0,    0,    1, -254, 400,  900, -10, -100};
...
void testAdd4Int8() {
  ...
  int8_t as[]   = {127, -1,  1, -128, -1, 5};
  int8_t bs[]   = {127, -1, -1, -128, 0, -5};
  int8_t cs[]   = {127, -1,  1, -128, 1, -1};
  int8_t ds[]   = {127, -1, -1, -128, 0, -1};
  int results[] = {508, -4,  0, -512, 0, -2};  
  const int numTests = sizeof(results)/sizeof(int);
...
void testAverage() {
  ...
  byte  as[]      = {0, 255, 127,  1, 0,   5, 73};
  byte  bs[]      = {0, 255, 128,  0, 1, 100, 59};
  byte  results[] = {0, 255, 127,  0, 0,  52, 66}; // answers are in lower bound
